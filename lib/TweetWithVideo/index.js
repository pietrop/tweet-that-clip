 ...
 **/
const TweetWithVideo = function (data, cb) {
  ...
  // making text optional
  if(data.tweet_text === undefined){
    self.tweet_text = "";
  }
  else{
 ...
 */
TweetWithVideo.prototype.tweet = function () {
  ...
  request.post({url: POST_TWEET_URL, oauth: self.OAUTH, form: request_data}, function(error, response, body) {
    // console.log('TWEETED-FINE: ', error, body)
    if(error !== null){
      if(self.cb){self.cb(error, null)};
    }
    else{
      let data = JSON.parse(body);
      if(self.cb){self.cb(null, data)};
    }    
  });
}
