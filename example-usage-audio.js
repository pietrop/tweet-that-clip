/**
 *  Example audio usage, with with captions
 */
const tweetThatClip = require('./index.js');
const path = require('path');

const opts = {
  inputFile: path.join(__dirname,'./assets/test.mp3'),
...
tweetThatClip(opts)
  .then((res)=>{
    console.log('in example-usage for audio',res);
  })
  .catch((error) => {
    console.log('Error in example-usage for audio',error);
  })
